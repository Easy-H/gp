...
import type { PanelLayoutColors } from './PanelGroup';
  ...
  return (
    <PanelGroup
      ...
      renderTabLabel={(wrapped, isActive) => {
        ...
        return <span style={{ fontSize: '12px', color: '#e7ebf3', fontWeight: isActive ? 'bold' : 'normal' }}>{data.title ?? data.name ?? wrapped.id}</span>;
      }}
