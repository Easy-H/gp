...
export const extractSizes = (layoutObj: SerializedPanelLayout | undefined) => {
  const colSizes: LayoutSizes = {};
  const rowSizesMap: Record<number, LayoutSizes> = {};
  if (layoutObj?.groups) {
...
export function usePanelSizeManager(
  ...
  const [colSizes, setColSizes] = useState<LayoutSizes>(() => extractSizes(layout).colSizes);
  const [rowSizesMap, setRowSizesMap] = useState<Record<number, LayoutSizes>>(() => extractSizes(layout).rowSizesMap);
  ...
  const onPanelDrop = useCallback((cIdx: number, rIdx: number) => {
    if (dropZone && dropZone !== 'center') {
      if (dropZone === 'left' || dropZone === 'right') {
        ...
        const nextColSizes: LayoutSizes = {};
        ...
        if (currentColRows && currentColRows[rIdx]) {
          ...
          const currentRowHeights: LayoutSizes = rowSizesMap[cIdx] || {};
          
          const heights: LayoutSizes = {};
