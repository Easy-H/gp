...
현재 관계 추출은 다음 기준을 우선으로 사용합니다.

- 멤버 변수: 클래스가 보유하는 의존 또는 구성 관계
- 함수 매개변수: 메서드 호출 중 협력하는 연관 관계
- `new` 생성: 추가적인 관계 신호로 보조 활용

특히 Spring Boot 같은 Java 코드에서는 `final` 필드나 주입된 멤버 변수를 중심으로 관계를 해석하는 편이 더 자연스럽습니다.
